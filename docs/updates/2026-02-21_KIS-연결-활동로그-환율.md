...
| `web/mini_server.py` | "즉시 매매" 버튼 → paper_trading 설정 무시, **KIS 연결 시 실매매** 실행 |
| `web/mini_server.py` | 디버그 URL 3개 추가: `/api/debug/kis-token`, `/api/debug/auto-trading-pipeline`, `/api/debug/fx-rate` |
| `web/mini_server.py` | **전체 활동 로그 미들웨어** — 모든 API 요청 자동 기록 + WebSocket 실시간 전송 |
| `web/mini_server.py` | **환율 실시간 갱신** — yfinance로 1시간마다 USD/KRW 자동 업데이트 (1450원 하드코딩 제거) |
| `web/mini_server.py` | 비서실장 위임 표시 제거 — 처장 이름만 직접 표시 |
