...
| 3 | 대기열 플러시 프로바이더 라우팅 오류 | 중간 | `web/mini_server.py` | `batch_submit()` → `batch_submit_grouped()` (프로바이더별 자동 분리) |
...
| 항목 | 결과 |
|------|------|
| mini_server.py `_SNS_PLATFORMS` | `["instagram", "youtube", "tistory", "naver_blog", "naver_cafe", "daum_cafe"]` — 정상 |
| mini_server.py에 linkedin/threads | 0건 — 정상 (제거 완료) |
...
`mini_server.py`에서 동일한 `/api/tasks/bulk` API가 2번 정의되어 있던 문제를 수정했습니다. FastAPI는 같은 경로의 API가 2개이면 두 번째를 무시하므로, 두 번째 정의(고급 기능 포함)가 작동하지 않았습니다. 기능을 합쳐서 1개로 통합했습니다.
...
(이전 커밋에서 수정된 파일: mini_server.py, ai_handler.py, db.py, index.html, agents.yaml/json, models.yaml, quality_rules.yaml/json, .env.example, CLAUDE.md)
