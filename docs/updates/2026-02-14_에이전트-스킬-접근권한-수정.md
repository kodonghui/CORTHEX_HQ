...
실제로는 `config/agents.yaml` 설정 파일에 에이전트마다 사용 가능한 도구가 명확히 지정되어 있었으나, 배포 서버(mini_server.py)가 이 설정을 무시하고 빈 배열(`[]`)을 반환하고 있었습니다.
...
| **`web/mini_server.py`** (배포 서버) | **항상 `allowed_tools: []`를 하드코딩**해서 반환 | **이게 원인!** |
| `web/templates/index.html` | 도구 이름 표시 함수 | `tool_id`가 아닌 `name`으로 검색 → 매칭 실패 |

## 변경 사항 요약

### 1. `web/mini_server.py` (배포용 경량 서버)
