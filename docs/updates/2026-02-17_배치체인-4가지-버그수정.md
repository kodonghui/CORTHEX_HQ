...
### 버그 2: specialists 배치 실패 시 오류 원인 불명 (arm_server.py)

| 구분 | 내용 |
|------|------|
| **위치** | `web/arm_server.py` — `_advance_batch_chain` specialists 단계 |
...
### 버그 3: synthesis 배치 실패 시 폴백 없음 (arm_server.py)

| 구분 | 내용 |
|------|------|
| **위치** | `web/arm_server.py` — `_advance_batch_chain` synthesis 단계 |
| **문제** | 종합보고서 배치가 실패하면 "응답 없음"으로 끝나버림 |
| **수정** | 배치 실패 시 `_synthesis_realtime_fallback()` 함수로 실시간 `ask_ai()` 호출 |

### 버그 4: 오류 추적 로직 부재 (arm_server.py)

| 구분 | 내용 |
|------|------|
| **위치** | `web/arm_server.py` — synthesis 단계 |
...
| `web/arm_server.py` | 오류 추적 강화 + `_synthesis_realtime_fallback` 함수 신규 추가 |
