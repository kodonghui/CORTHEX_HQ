...
- `mini_server.py`의 `_call_agent()`에서 에이전트별 허용 도구로 스키마를 로드하고 ToolPool을 통해 도구 실행
...
| `web/mini_server.py` | 피드백 토글 API + 도구 호출 연동 |
...
| `web/mini_server.py` | PENDING 추적 + 자동 결과 수집 폴러 + /배치실행/배치상태 명령 |
