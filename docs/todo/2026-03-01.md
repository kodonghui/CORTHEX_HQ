...
### 누나 CLI 자격증명 연동
- `/home/ubuntu/.claude-sister/.claude/.credentials.json` 생성 — 누나 Max OAuth 토큰 등록
- WebSocket 실테스트: Eden/Zoe/Sage 3명 전원 $0 응답 확인
- 대표님 에이전트 = 대표님 Max 한도, 누나 에이전트 = 누나 Max 한도 (완전 분리)

### 빌드 번호 자동화
- `index.html` 빌드 번호 `#730` 하드코딩 → `BUILD_NUMBER_PLACEHOLDER` 복원
- CSS/JS 캐시버스팅도 동일하게 placeholder 처리
- 이후 배포 시 `deploy.yml` sed가 자동으로 실제 번호 주입
