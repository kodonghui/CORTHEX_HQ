...
> 마지막 업데이트: 2026-03-03 (트레이딩 안전장치 3대 과제 완료)
...
- ⬜ **deploy-fast.sh에 yaml2json 추가 검토** — agents.yaml 수정 시 JSON 동기화 누락 위험. deploy-fast.sh에 python3 yaml2json.py 추가하거나, YAML 수정 시 로컬에서 수동 실행 규칙화

---

## 🔴 긴급 (데드라인 있음)

- ✅ ~~**트리거 무한누적 방지**~~ — 새 분석 시 기존 buy_limit 초기화 + 10분 미체결 자동 만료. 2026-03-03 실사고(34개 동시 발동) 해결
- ✅ ~~**CIO 분석에 포트폴리오 정보 주입**~~ — AI 프롬프트에 현재 보유종목/잔고/총자산 전달. `_build_portfolio_context()` 헬퍼 추가
- ✅ ~~**주문 시 전체자산 기반 비중 계산**~~ — 현금만→총자산 기준 + 현금 캡 적용. 잔고 조회 실패 시 100만원 폴백 제거
...
## ✅ sister 인증 격리 + 로그아웃 수정 (빌드 #759~#760)

- ✅ ~~**sister 인증 org 격리**~~ — auth_handler `_extract_token()` + `get_auth_org()` + 쿠키 토큰. 대화/작전일지/기밀문서 3핸들러 인증 필터 (PR #728, 빌드 #759)
- ✅ ~~**saju 에이전트 사이드바 표시**~~ — index.html "준비중" → 4명 실제 렌더링 (PR #728)
- ✅ ~~**비서실장 sister 숨김**~~ — `x-show="auth.role !== 'sister'"` (PR #728)
- ✅ ~~**로그아웃 버튼 bootstrapMode 수정**~~ — 로그인 성공 시 `bootstrapMode = false` (PR #729, 빌드 #760)

---

## ✅ v5 완료 (빌드 #750~#752)

- ✅ ~~**migrate_v5.py 실서버 실행**~~ — deploy.yml 통해 서버 실행 완료. org컬럼 4테이블 + sns_accounts 생성 (빌드 #751)
- ✅ ~~**FR-5/6: 채팅/기밀문서 org 필터링**~~ — DB org 컬럼 + API ?org= 파라미터 + JS sessionOrg 전달 (빌드 #752)
- ✅ ~~**FR-9: 통신국 org 스코프**~~ — `/api/sns/accounts?org=saju` 엔드포인트 + saju Instagram 플레이스홀더 (빌드 #752)
- ✅ ~~**FR-10: 작전일지 org 스코프**~~ — activity-logs API org 파라미터 + UI 전달 (빌드 #752)
- ✅ ~~**FR-11: @멘션 org 라우팅**~~ — `_AGENT_DISPLAY_NAME_TO_ID` 역매핑 + @Eden/@Zoe/@Sage 라우팅 (빌드 #750)
- ✅ ~~**saju 에이전트 소울 파일**~~ — saju_eden.md (Eden), saju_zoe.md (Zoe), saju_sage.md (Sage) 작성 (빌드 #752)
