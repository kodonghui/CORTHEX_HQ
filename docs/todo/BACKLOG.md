...
> 마지막 업데이트: 2026-02-28 (빌드#689 — SketchVibe Phase 3 + SNS 리서치)
...
- ⬜ **티스토리 자동 발행 — 실제 글 발행 테스트** — 카카오 로그인 + 글쓰기 접근 ✅ 확인. 에디터 iframe + 태그 입력 발견. 실제 테스트 글 발행 필요. 쿠키 재사용으로 카카오톡 인증 매번 안 해도 되는지 확인
- ⬜ **다음카페 자동 발행 — 글쓰기 접근 + 발행 테스트** — 카카오 로그인 ✅, 카페 접근 ✅. 글쓰기 페이지 + 에디터 셀렉터 확인 + 실제 테스트 글 발행 필요
