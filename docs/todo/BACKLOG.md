...
> 마지막 업데이트: 2026-02-28 (SNS 자동발행 — 티스토리 공개 발행 성공)
...
- ✅ ~~**티스토리 자동 발행**~~ — 공개 발행 성공! 테스트 글 3건 (비공개2+공개1). `tistory_publisher.py` 전면 수정 완료. 쿠키 재사용 ✅ (카카오톡 인증 없이). 핵심: ActionChains label 클릭으로 React 공개 라디오 전환
- ⬜ **다음카페(서로연) 자동 발행 — 글쓰기 URL 수정 + 발행 테스트** — 카카오 로그인 ✅. 글쓰기 URL 형식이 다름: 기존 `_write?board_id=` → 실제 `_c21_/united_write?grpid=1ApWV`. `daum_cafe_publisher.py` URL 수정 + 에디터 셀렉터 탐색 + 테스트 발행 필요
