...
### 노션 연동
- ⬜ 노션 보고 구조 결정: 서버(mini_server.py)가 자동 업로드
- ⬜ 에이전트 직접 노션 사용: 비서실장 + CPO + 연대기 + 아카이브 + 보고서 (5명만)
- ⬜ web/notion_client.py 구현 (Notion API 클라이언트)
- ⬜ mini_server.py에 노션 엔드포인트 추가
- ⬜ 자동 업로드 트리거 결정 (AI 응답 완료 시? 일정 조건?)
- ⬜ 대시보드에 노션 연동 상태 표시
