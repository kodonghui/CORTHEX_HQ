...
- **작업 내용**: 멀티에이전트 브로드캐스트 + 상태 표시등 + 자동 스크롤
- **빌드 번호**: #129
...
5. **처장 → 전문가 하위 위임 + 품질 검수 시스템**
6. **노션 API 키 등록 + 보고 시스템 활성화**
7. **SNS API 키 등록** (인스타그램, 유튜브 등) — 실제 게시 기능 활성화
8. 도메인(corthex.com 등) 연결 → IP 주소 대신 이름으로 접속
9. HTTPS(보안 연결) 설정 → Let's Encrypt 무료 인증서
10. **서버 업그레이드 검토** — AWS 또는 Oracle ARM 4코어 24GB 무료 티어
