...
- **날짜**: 2026-02-15
- **버전**: `0.07.001`
- **작업 브랜치**: claude/fix-8-uxui-improvements
- **작업 내용**: UX/UI 10가지 개선 — 다크모드 밝기↑, 보내기 버튼 가시성↑, 패널 전환 애니메이션, 아카이브 카드형 디자인, CEO 클릭, 예산 대시보드 표시, 빌드 카드 정렬, 로고 홈 이동, 입력란 구분 강화, 대화 내보내기 위치 개선
...
- ~~데이터 영구 저장 필요~~ → **완료** (0.07.000에서 SQLite DB 도입)
- **Phase 2 예정**: 비서실장 AI를 텔레그램에 연결하여 CEO 명령을 실제로 AI가 처리하도록
...
1. **Phase 2: 비서실장 AI 연결** — 텔레그램으로 CEO가 보내는 명령을 비서실장 AI가 실제로 처리
2. **Phase 3: 웹 로그인 + PUT/POST API** — 간단한 비밀번호 로그인, 설정 저장 기능
3. **Phase 4: 스마트 모델 라우팅 + 비용 제한** — 작업 난이도에 따라 AI 모델 자동 선택, 일일 비용 10,000원 제한
4. **29명 에이전트 소울(성격) 설정** — `docs/TODO-에이전트-설정.md` 참고
5. **노션 API 키 등록 + 보고 시스템 활성화**
6. **SNS API 키 등록** (인스타그램, 유튜브 등) — 실제 게시 기능 활성화
7. 도메인(corthex.com 등) 연결 → IP 주소 대신 이름으로 접속
8. HTTPS(보안 연결) 설정 → Let's Encrypt 무료 인증서
9. **Oracle Cloud 서버 업그레이드** — ARM 4코어 24GB 무료 티어로 이전 시도
