...
| # | 내용 | 설명 |
|---|------|------|
| 6-5 | **ARGOS 데이터 수집 레이어** | 주가/뉴스/공시/경제지표 1분~1시간 주기 자동 수집 + DB 누적 (90일). 분석 시 API 호출 0회 |
| 6-6 | **도구 서버화 Phase 2** | DART API + Naver News + ECOS 서버 직접 호출. AI 토큰 70% 절감 목표 |
| 6-7 | **신뢰도 완전 서버화** | Quant(①)+Calibration(②)+Bayesian(③)+ELO(④) → 서버 수식. AI는 뉴스 맥락 ±15%p만 |
| 6-8 | **정보국 탭 + 상단 상태바** | 상태바(항상보임): 데이터✅ AI✅ 트리거N건 비용$X. 정보국 탭: 시스템상태+트리거목록+교신로그+비용추적+에러로그 |
| 6-9 | **강화학습 파이프라인 정비** | 서버: ELO/Platt/Bayesian 수치 업데이트. AI: 오답 패턴 해석 주기 리포트(월 1회) |
