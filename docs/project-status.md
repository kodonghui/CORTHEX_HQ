...
- **날짜**: 2026-02-15
- **작업 브랜치**: claude/fix-config-loading
- **작업 내용**: 에이전트 스킬 표시 문제 완전 해결 — 서버 설정 파일 경로 + PyYAML 환경 문제 수정
...
- ~~다크모드 텍스트 안 보이는 문제~~ → 순수 CSS 색상 시스템으로 완전 해결 — 194개+ 클래스를 하드코딩, Tailwind CDN 의존성 제거 (2026-02-15)
- ~~빌드 번호 불일치 (#399 vs #37)~~ → mini_server.py의 git 커밋 개수 방식 제거, deploy.yml 번호로 통일 (2026-02-15)
- ~~에이전트 스킬 탭에서 모든 에이전트가 "도구 없음"으로 표시~~ → 서버 설정 파일 경로 수정 + JSON 폴백 추가로 완전 해결 (2026-02-15)
