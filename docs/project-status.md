...
- **날짜**: 2026-02-14
- **작업 브랜치**: claude/fix-agent-skill-access-i6i6D
- **작업 내용**: 에이전트 스킬(도구) 접근 권한 표시 수정 — mini_server.py가 agents.yaml의 allowed_tools를 실제로 반환하도록 수정
...
5. **테마(다크/라이트) 가시성 전면 수정** - CSS 변수 기반 자동 테마 전환, 하드코딩 색상 30곳+ 교체, 대비 개선
