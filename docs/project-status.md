...
| 변경 | 내용 |
|------|------|
| 트리거 무한누적 방지 | 새 분석 시 기존 buy_limit 전부 비활성화 + 10분 미체결 자동 만료 |
...
| 변경 | 내용 |
|------|------|
| Drawflow 완전 제거 | CDN, 초기화, 노드추가, 저장/불러오기, 변환 코드 전부 삭제 |
| Mermaid 네이티브 캔버스 | Mermaid.js가 직접 SVG 렌더링, 코드 = 진실의 원천 (SSOT) |
| 변환 0번 아키텍처 | 사용자가 보는 것 = Claude가 읽는 것 = 저장되는 것 |
| 노드 8종 | agent/system/api/decide/db(신규)/start/end/note |
| SVG 인터랙션 | 클릭 선택, 더블클릭 편집, Delete 삭제, 연결 모드, 엣지 삭제 |
| 백엔드 간소화 | Drawflow 파싱 ~150줄 제거, Mermaid 직접 저장/반환 |
| 레거시 호환 | 기존 Drawflow JSON 자동 마이그레이션 |

## 2026-03-02 — SketchVibe Mermaid 파서 + 빠른 배포 세팅 (빌드 #814~#815)

| 수정/추가 | 내용 |
|------|------|
| Mermaid→Drawflow 파서 버그 | 인라인 노드 정의(`A([시작])-->B{결정분기}`) 파싱 실패 → B,C,D,E가 ID를 레이블로, 전부 system 타입 폴백. `parseNodeToken` 헬퍼로 수정 |
| `[[label]]` 이중 대괄호 지원 | subroutine 모양 → system 타입 추가 |
| SSH 빠른 배포 스크립트 | `deploy-fast.sh` 추가 — GitHub Actions(3분) 대신 SSH 직배포(30초) |
| CLAUDE.md 워크플로우 | STEP5에 빠른/일반 배포 구분 명시 |

## 2026-03-02 — NEXUS 버그 2개 수정 (빌드 #812~#813)

| 수정 | 내용 |
|------|------|
| NEXUS 재열 시 캔버스 로딩 안 됨 | `x-if`로 DOM 파괴 시 stale canvasEditor 문제 → `openNexus()`에서 매번 재초기화 |
| 노드 이름 변경 저장 안 됨 | Drawflow `export()`가 내부 상태 읽음 → `commit()`에서 `nd.html + nd.data.label` 직접 업데이트 |

## 2026-03-02 — E2E 소크라테스 QA + 추가 버그 2개 (빌드 #808~#809)

| 수정 | 내용 |
|------|------|
| CLAUDE.md 워크플로우 | E2E 소크라테스 QA STEP 1~6 전면 개편. 버그 수정 시에도 동일 적용 명시 |
| savedCanvasName 버그 | `newNexusCanvas()` 서버 저장 후 `savedCanvasName` 미갱신 → 새 캔버스 이름 변경 시 이전 파일 미삭제 수정 |
| 참고 자료 flowcharts 노출 | `loadKnowledge()` 에서 `flowcharts/` 폴더 필터 추가 → NEXUS 캔버스 파일 참고 자료 탭에서 제외 |
...
| 수정/기능 | 내용 | 빌드 |
|---------|------|------|
| Mermaid 오버레이 색상 | 남색 하드코딩 → hq-* 토큰. 캔버스로 돌아가기 버튼 추가 | #787 |
| SketchVibe 간소화 | 중복 UI(캔버스파일명/세션시작) 제거. 내가그리기 탭 삭제 | #786 |
| 캔버스 삭제 버튼 | 저장된 캔버스 hover 시 × 삭제 버튼 | #785 |
| 캔버스 목록 버그 | data.files→entries, .md→.json 포함, Delete키 노드삭제 | #784 |
| 화살표 라벨 | 더블클릭 → 설명 입력 → 중앙 표시. 저장/불러오기 포함 | #783 |
| 다이어그램 자동표시 | NEXUS 열 때마다 옛날 다이어그램 튀어나오는 버그 수정 (2분 룰) | #782 |
| 팔레트 노드 | 뷰포트 중앙 기준 배치 + editor null toast | #781 |
| SketchVibe UX | 맞아 버튼 하단 중앙 + 새로 개선 | #780 |
| Mermaid 렌더링 | HTML div `>` 누락 + SSE 단절 버그 | #779 |

## 2026-03-02 — SketchVibe 버그 수정 + UX 전면 개편 (빌드 #779~#781)

| 수정 | 내용 |
|------|------|
| Mermaid 렌더링 버그 | HTML div `>` 누락 → DOM 붕괴 수정 |
| SSE 단절 | 패널 닫기와 SSE 분리 |
| 노드 위치 | 고정 좌표 → 뷰포트 중앙 기준 |
| + 새로 | r.ok 체크 추가 + 리스트 즉시 반영 |
| UX 개편 | 모드 탭(내가 그리기/같이 그리기) + 맞아 버튼 하단 중앙 |

상세: `docs/updates/2026-03-02_SketchVibe_버그수정_UX개편.md`
...
| 수정 | 증상 | 방법 |
|------|------|------|
| workspace-profile 404 | viewer 롤 → 404 → sidebarFilter 기본값 → 에이전트 안 보임 | viewer 프로파일 추가 + 폴백 반환 |
| 사이드바 에이전트 | CEO 로그인해도 비서실장만 보임 | initWorkspace 토큰 없으면 스킵 |
| 대시보드 비용 | 누나 화면에 $204.86 노출 | orgScope 기반 에이전트 비용만 집계 |

상세: `docs/updates/2026-03-02_v52_workspace격리_사이드바복구_대시보드비용격리.md`
...
| PR | 빌드 | 내용 |
|----|------|------|
| #724 | #750 | 에이전트 리네임 + CLI 라우팅 보호 + batch 제거 |
| #726 | #751 | DB 마이그레이션 (org 컬럼 + sns_accounts) |
| #727 | #752 | org 스코프 FR-5~11 완성 + saju 소울 파일 |

**v5 완료 항목**: 에이전트 ID 리네임 · saju 본부 · `_can_command()` · DB org 컬럼 · 채팅/기밀문서/로그 org 필터 · @멘션 라우팅 · saju 소울(Eden/Zoe/Sage)
...
| 빌드 | 내용 |
|------|------|
| #745 | PR #721 — 인디고 B: #171730 미디엄 인디고 + violet-500 |

### 현재 팔레트 (인디고 B)
- 배경: `#171730` (미디엄 인디고)
- 액센트: `#8b5cf6` (violet-500)
- 카드: `#21213e` / 패널: `#1b1b38`
- body glow: 미세한 바이올렛 radial-gradient

### 다음 작업 (UX 대개편 계속)
- Phase 2: 대시보드 카드 계층감 / 사이드바 에이전트 카드 개성
- 로그인 화면 Noir 스타일 정합성 확인
- BACKLOG: v5 누나 에이전트, CLAUDE_API_KEY_SISTER 등록
...
| 빌드 | 내용 |
|------|------|
| #740 | PR #715 — 다크모드 인디고 팔레트 (bg #1e1e40, violet-400) |
| #741 | PR #716 — 로그인 화면 풀스크린 재설계 + 항상 표시 (bootstrap mode 우회 버그 수정) |
...
## 2026-03-01 — UX 대개편 Phase 2 + v5 + 피드백 버그 (빌드 #733~#739)
...
| #736 | 다크모드 밝기 추가 상향 (bg #1e1e26) |
| #737 | ✅ PR #712 — v5 로그인/3본부/기밀문서탭/CLI 라우팅 배포 완료 |
| #739 | ✅ PR #714 — 피드백 핀 버그 수정 + E2E 테스트 13개 배포 완료 |

### v5 배포 내용 (빌드 #737)
- v5-1: 로그인 역할 탭 (대표님/누나) + 역할별 비밀번호
- v5-2: agents.yaml org/cli_owner 필드
- v5-3: 사이드바 3본부 구조 (리트마스터/스케치바이브/사주)
- v5-4: 기밀문서 본부별 탭 + matchesBunbu() 필터
- v5-5: `CLAUDE_API_KEY_SISTER` CLI 라우팅

### v5 전략 확정 (Party Mode)
- 예창패: 리트마스터로 확정
- 스케치바이브: CORTHEX CLO에게 임시 출원 위임
- 3본부 체제: 리트마스터 / 스케치바이브 / 사주
- 누나 합류: 사주 본부 관리 + 별도 CLI 연동

### 다음 작업
- 대표님 corthex-hq.com 실사용 후 피드백 반영
- 다크/라이트 모드 경고창 원인 파악 (런타임 재현 필요)
- Phase 3 채팅버블/대시보드 카드 (선택)
- 사주 본부 에이전트 추가 (누나 합류 시)
- CLAUDE_API_KEY_SISTER GitHub Secrets 등록 (누나 계정 생기면)
