...
- **작업 내용**: index.html UX 전수검사 8개 이슈 수정 (부서필터/스크롤/피드백/2단레이아웃/Pretendard/이미지/네모배지)
- **빌드 번호**: #139
...
7. **키움증권 Open API+ 연동** — 모의투자 → 실거래 전환 (API 키 발급 필요)
8. **노션 API 키 등록 + 보고 시스템 활성화**
8. **SNS API 키 등록** (인스타그램, 유튜브 등) — 실제 게시 기능 활성화
9. 도메인(corthex.com 등) 연결 → IP 주소 대신 이름으로 접속
10. HTTPS(보안 연결) 설정 → Let's Encrypt 무료 인증서
11. **서버 업그레이드 검토** — Oracle ARM 4코어 24GB 무료 티어(158.179.165.97 생성 완료, SSH 키 등록 필요)
