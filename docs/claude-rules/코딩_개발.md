...
## 🔴🔴 소크라테스 QA — UI 작업 후 반드시 셀프 시행!

> BDD 변형. 구현 후 내가 먼저 기댓값을 말하고, 실서버에서 확인한다.

### 방법
```
시나리오: [A] → [B] → [C]
기댓값: [결과]
실제: 확인 → 일치? → OK / 불일치? → self-doctor
```

### 시나리오 작성 기준
- 핵심 플로우 전부 (저장/불러오기/삭제/새로만들기/이름변경)
- 엣지케이스 포함 (이름 변경 후 저장 / 빈 상태 / 연속 동작)

### 불일치 발생 시 → Self-Doctor (보고 없이 자율 수행)
1. `investigator` 에이전트 → 원인 파악
2. 수정 → `spec-validator` 검증
3. 통과 후 커밋. 대표님 보고는 선택

### 예시
```
시나리오: +새로 → 이름 "테스트a"로 변경 → 저장
기댓값: "테스트a" 세션만 목록에 표시
실제: "새 캔버스 1" + "테스트a" 둘 다 있음 → 버그 → self-doctor
```
