...
**이번 검사에서 수정하지 않은 이유**: `src/core/` 범위를 벗어난 변경(`.gitignore`, `web/mini_server.py` 수정)이 필요하므로 팀장에게 보고.
...
실제 도구 자동호출 루프(Function Calling Loop)는 `web/mini_server.py`의 `_call_agent()` 함수에 구현되어 있습니다.
...
즉, 예산이 초과되어도 에이전트 명령은 계속 실행됩니다. `check_warning()`을 호출해서 경고 메시지만 반환할 뿐, 실제로 실행을 막지는 않습니다. 실제 차단 로직이 `web/mini_server.py`에 있는지 별도 확인이 필요합니다.
...
단, `mini_server.py`에서 일부 데이터를 DB로 마이그레이션하는 로직이 있을 수 있으므로 BE 팀에서 추가 확인 필요합니다.
...
2. async/await 오류 0건
3. 경고 사항들은 `src/core/` 범위를 벗어난 변경(`.gitignore`, `mini_server.py`, SQLite 마이그레이션)이 필요하여 팀장에게 보고
...
| 우선순위 | 이슈 | 위험도 | 필요 작업 |
|---------|------|--------|---------|
| 높음 | 예약/워크플로우/프리셋 YAML 저장 → 배포 시 날아갈 수 있음 | 높음 | `mini_server.py`에서 SQLite 마이그레이션 확인 |
...
| 중간 | 예산 초과 시 에이전트 자동 중단 없음 | 중간 | `mini_server.py`에서 예산 체크 후 차단하는지 확인 |
...
- **예산 초과 차단: src/core/에는 없음 (mini_server.py에서 확인 필요)**
