...
## 💡 #007: OpenClaw 서버 통합 아이디어 — "오픈클로한테 시키면 걔가 직원들 스폰하고 이러지 않을까?"

**날짜**: 2026-02-22
**분류**: 아키텍처 설계 / 인프라 확장
**영향**: CORTHEX 접근성 + 자동화 수준 대폭 향상 가능

### 대화

> **CEO**: 오픈클로 알아? 스케줄링 이런거 지금 코드 존나 꼬여서 다른 브런치에서 웹 전체 리팩토링중이거든?
> 근데 막 어렵게 @이런 기능 할거없이 오픈크로한테 시키면 걔가 직원들 스폰하고 이러지 않을까?
> 그리고 서버24시간 도니까 컴퓨터 켜놓을 필요도 없고 ㅋㅋㅋ
> 한번 오픈클로 최신 기술을 최대한 살펴보고 분석해서 우리 서버에 넣으면 어떤 재밌는걸 쉽게 할 수 있을지 생각해볼래?
>
> **CEO** (추가): Claude Agent SDK는 뭔데? 그리고 클로드 코워크를 서버에 넣을수는 없나?
>
> **Claude**: OpenClaw(구 Clawdbot/Moltbot)은 MIT 라이선스 완전 무료 오픈소스 AI 에이전트 플랫폼입니다.
> GitHub 별 16만개, ARM 서버에서 바로 돌아갑니다.
> 핵심: 텔레그램/왓츠앱/디스코드 등 8개+ 채팅앱에서 하나의 AI에게 지시할 수 있고,
> 멀티에이전트 라우팅(채널별로 다른 직원이 응답)이 기본 탑재.
> 하트비트(30분마다 알아서 할 일 체크하는 능동적 AI)도 있습니다.
>
> Claude Cowork는 데스크탑 전용이라 서버에 못 넣지만,
> Claude Agent SDK는 서버에서 headless로 실행 가능합니다.

### 핵심 통찰

CORTHEX는 이미 29개 에이전트 + 131개 도구를 가진 강력한 시스템이지만,
**접근 방법이 "웹 + 텔레그램" 2개에 한정**되어 있다.

CEO의 아이디어: OpenClaw을 CORTHEX 서버에 설치하면
1. **멀티채널**: 텔레그램뿐 아니라 왓츠앱, 디스코드, 슬랙 등 모든 채팅앱에서 접근
2. **에이전트 스폰**: @CIO, @CMO처럼 채팅에서 직접 에이전트 호출
3. **하트비트**: 시키지 않아도 30분마다 알아서 점검/보고
4. **24시간 서버**: 컴퓨터 안 켜도 서버가 항상 대기

### OpenClaw 기술 조사 결과

| 항목 | 내용 |
|------|------|
| **정체** | 오픈소스 자율형 AI 비서 (MIT 라이선스, 완전 무료) |
| **GitHub** | 16만+ 별, 2만+ 포크 (역대 최고 기록) |
| **만든 사람** | Peter Steinberger → 2026.2.14 OpenAI 입사, 프로젝트는 오픈소스 재단으로 이관 |
| **채널** | 텔레그램, 왓츠앱, 디스코드, 슬랙, 시그널, iMessage, Teams, Google Chat |
| **기능** | 멀티에이전트 라우팅, 크론, 하트비트, 스킬 마켓(5,700+), Soul 설정 |
| **서버 요구** | Node 22+, 최소 2코어/2GB (Oracle ARM 4코어/24GB면 넉넉) |
| **비용** | 소프트웨어 무료, AI API 비용만 ($5-25/일) |
| **보안 주의** | ClawHub 악성 스킬 341개 발견 → 검증된 스킬만 사용 필수 |

### 추천: 단계적 통합

| 단계 | 작업 | 시기 |
|------|------|------|
| 1단계 | 리팩토링 완료 (현재 진행 중) | 먼저 |
| 2단계 | OpenClaw 서버 설치 + 텔레그램 연결 | 리팩토링 후 |
| 3단계 | CORTHEX 29개 에이전트를 OpenClaw 스킬로 연결 | 2단계 후 |
| 4단계 | 하트비트 + 멀티채널 확장 | 3단계 후 |

### 같이 조사한 기술

| 기술 | 설명 | 서버 실행 |
|------|------|-----------|
| **OpenClaw** | 오픈소스 AI 비서 (채팅앱 통합) | ✅ 가능 |
| **Claude Agent SDK** | 코드에서 Claude를 자동으로 부르는 리모컨 | ✅ 가능 |
| **Claude Cowork** | 비개발자용 데스크탑 AI 에이전트 | ❌ 데스크탑 전용 |

### 결과

아이디어 단계. 리팩토링 완료 후 구체적 실행 계획 수립 예정.
