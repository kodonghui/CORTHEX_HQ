...
## 💡 #009: NEXUS — "자비스처럼 시스템을 마우스로 탐색하면서 Claude와 논의"
...
> **CEO**: 홈페이지 전체 기능을 플로우차트로 그려놓으면 어떨까? 뼈대가 있어야 가지를 붙이기 쉽잔아.
> 혹은 내가 그 플로우차트를 마우스로 이어서 어떻게 작동하라는건지 이해시킨다던가 말이야.
>
> **Claude**: 레퍼런스 분석 후 3가지 모드 제안: 2D 에디터, 3D 뷰, 비주얼 캔버스.
> 이름은 NEXUS (Network EXploration & Unified System)를 추천합니다.
>
> **CEO**: NEXUS 좋다 미친 넥서스 ㅋㅋㅋ
> 3D에는 각가지 기능들 세분화해서 싹 연결해놔야지. 이름을 써놔야지 정사각형으로 하던가.
> 그리고 NEXUS를 더보기에 넣지 말고 채팅/사무실/대시보드 거기 옆에 넣고 전체화면으로 보게 해줘.
> 조직도 탭은 삭제. NEXUS 3D로 대체.

### 핵심 통찰

**비전**: 대표님이 마우스로 시스템을 탐색하고, 노드를 그리고 연결하면서 Claude와 시각적으로 논의하는 "자비스 공간".

**구현**:
- 전체화면 오버레이 (채팅/사무실/대시보드 옆 NEXUS 버튼)
- 3D 시스템 맵: 7개 카테고리 70+ 노드 (탭, 부서, 에이전트, API, 저장소, 서비스, 프로세스)
- 비주얼 캔버스: 노드 드래그 + 연결 + 더블클릭 이름 수정 + JSON 저장
- 조직도 탭 삭제 → NEXUS 3D가 완전 대체

**재사용**: `docs/architecture/nexus-design.md`에 패턴 정리. 다른 프로젝트에서도 적용 가능.
