...
## 전문가 지식 통합 (저작권·특허/약관 2명 흡수)

### 저작권
- **공정이용 4요소** (저작권법 §35조의5): ①이용목적(영리성·변형적) ②저작물성격(사실/창작) ③이용양·질 ④시장영향(대체효과). 4요소 종합 — 1개만으로 결론 불가
- **CC License Matrix**: BY→NC→ND 순으로 제한 강화. AI 학습에는 CC BY/CC BY-SA/CC0만 안전. NC 없으면 상업적 OK
- **EU AI Act Art.53**: 범용 AI = 학습 데이터 저작권 준수 + 출처 문서화 필수. robots.txt + 저작권 고지 확인
- 추가 도구: `trademark_similarity` (상표 유사도 검사)

### 특허·약관
- **IPR Landscaping 3단계**: ①Patent Map(KIPRIS 분포) → ②White Space(미출원 영역) → ③FTO(타사 침해 여부). 新기술 출시 전 필수
- **Privacy by Design 7원칙** (원칙3): 기본값 프라이버시(opt-in). opt-out 설계 = 개보법 위반
- **한국 플랫폼 법령 2024**: 전자상거래법(청약철회 14일)+약관규제법(일방면책 무효)+개인정보보호법(최소수집)+정통망법(14세 미만 법정대리인 동의)
- **계층적 약관**: Layer1(요약 5줄)+Layer2(법적 필수)+Layer3(기술 세부). 문장 15단어 이하
- 추가 도구: `scholar_scraper action=search, query="선행기술 학술논문"` (특허 선행기술 조사)
