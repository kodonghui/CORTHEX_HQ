...
#### 2단계: 비용 vs 효과 최적화 — 근거 있는 수치만

**팀 편성 전 반드시 이 계산을 먼저 한다. 팀 뽑고 나서 계산하면 이미 늦음.**

##### 근거 1: Amdahl's Law — 병렬화 이론 상한 (1967, 실증 검증됨)

```
S(N) = 1 / ((1 - P) + P/N)
```
- P = 병렬화 가능 비율. **전수검사 P값은 실측 없음 — 아래 수치는 추정**
  - 파일 읽기(병렬 OK): P ≈ 0.8 추정
  - 결과 통합/커밋(순차 강제): P = 0 (무조건 팀장 혼자)
- 이론적 최대 향상(P=0.8 기준): N→∞일 때 **5배가 천장**. N=4면 이미 천장의 80% 달성

##### 근거 2: LLM 멀티에이전트 실증 연구 — **2024년 최신**

**논문**: "Towards a Science of Scaling Agent Systems"
**출처**: Google Research + MIT, arXiv:2512.08296 (2024년 12월)
**실험**: 4 벤치마크 × 5 아키텍처 × 3 LLM = 180개 설정 통제 실험

| 측정 항목 | 실측 수치 |
|----------|---------|
| 오케스트레이터(팀장) 없이 병렬만 → 오류 증폭 | **17.2배** |
| 오케스트레이터 있는 구조 → 오류 증폭 | **4.4배** |
| 병렬 분해 가능 작업에서 성능 향상 | **+80.8%** |
| 순차 추론 작업에 에이전트 추가 시 성능 | **-39% ~ -70%** |
| 도구 10개 초과 환경에서 에이전트 효율 | **2~6배 하락** |

→ 파일 읽기 = 병렬 분해 가능 → 멀티에이전트 +80.8%. **읽기 팀원, 판단/커밋은 팀장 혼자.**
→ CORTHEX 도구 154개 환경 = "도구 10개 초과" 경고 해당. 에이전트 많이 붙일수록 효율 하락.

##### 근거 3: 에이전트 소통 오버헤드 증가율 — **2024년 실측**

**출처**: arXiv:2511.02755 (2024년 11월)

```
소통 오버헤드 ∝ N^1.724  (실측값, 선형이 아님)

N=2 팀원: 오버헤드 2^1.724 = 3.3배
N=5 팀원: 오버헤드 5^1.724 = 14.8배
N=9 팀원: 오버헤드 9^1.724 = 43배
N=10 팀원: 오버헤드 10^1.724 = 53배 ← 여기서 조율 비용이 실제 작업 능가
```

멀티에이전트는 단일 에이전트 대비 입력 토큰 **4~220배** 더 사용 (같은 논문 실측)

##### 결론 — 팀 편성 기준표

```
서브에이전트 수 (Amdahl 이론 + 2024 실증 기반):
- 파일 ~500줄   → 2명 (P=0.8 기준 1.67배 향상, 가성비 최고)
- 파일 ~3000줄  → 3명 (2.14배 향상)
- 파일 ~5000줄  → 4명 (2.50배 향상, N^1.724 오버헤드 시작 전)
- 5000줄 초과   → 4명에서 멈출 것 (4명 이상은 수확체감 + 오버헤드 급등)

팀원 수 (N^1.724 오버헤드 기준):
- N=9에서 오버헤드 43배 → 실용 상한은 7~9명
- 전체 전수검사: 파일 수 기준 팀원 배정, 최대 9명 + QA 1명
```

| 작업 규모 | 팀원 수 | 서브에이전트 | 근거 |
|----------|--------|------------|------|
| 단일 버그 | 1~2명 + QA | 2명 | 팀 오버헤드 > 이득 |
| 기능 영역 점검 | 3~5명 + QA | 3명씩 | N^1.724 최소 구간 |
| 전체 전수검사 | 파일 수 기준, 최대 9명 + QA | 파일 크기별 3~4명 | 9명 초과 시 오버헤드 43배 |
| 도구 154개 검사 | TOOL 1명 + 서브 4명 | 4명 그룹별 | 도구 10개 초과 효율 하락 |
...
**필수 포함 항목 체크리스트:**
- [ ] **절대경로 명시**: `c:\Users\elddl\Desktop\PJ0_CORTHEX\CORTHEX_HQ\CORTHEX_HQ\web\mini_server.py` 형식으로 전체 경로
- [ ] **담당 파일 명시 + "이 파일만 수정"** 강조 (다른 파일 건드리면 충돌)
- [ ] **수정할 함수/클래스 이름** 명시: "파일 전체가 아니라 `_call_agent()` 함수 안의 progress 계산 부분만 수정"
- [ ] **수정 전/후 코드 예시** 제공: ❌ `progress = 90` → ✅ `progress = (tool_calls / 5) * 100`
- [ ] **왜 이 수정이 필요한지** 한 줄 설명 (맥락 없으면 팀원이 잘못 해석)
- [ ] **건드리면 안 되는 것** 명시: "division, model, tools 필드 절대 수정 금지"
- [ ] **완료 확인 방법** 제시: "grep으로 구버전 문자열 0건 확인" 또는 "yaml2json.py 실행 후 JSON 확인"
- [ ] **서브에이전트 활용 방법** 구체적으로: "파일 구간을 1~2000줄 / 2001~4000줄 / 4001~끝 으로 나눠서 3개 서브에이전트 병렬 읽기"
- [ ] **결과 저장 경로**: `docs/updates/fe_changes.md` 에 저장 후 한 줄 요약만 보고
- [ ] **허용/금지 모델명 목록** (CLAUDE.md "실제 존재하는 모델명" 표 복사해서 붙여넣기)

**좋은 지시 예시 vs 나쁜 지시 예시:**

❌ 나쁜 예 (러프함):
```
mini_server.py에서 진행률 수정해줘. 도구 호출 기반으로 바꿔.
```

✅ 좋은 예 (구체적):
```
파일: c:\...\web\mini_server.py
수정 대상: _call_agent() 함수 내부 (약 1200~1350줄 구간)
문제: agent_status WebSocket 이벤트에서 progress가 고정값(90)으로 전송됨
수정 방법:
  - tool_calls 카운터 변수 추가 (함수 시작 시 0으로 초기화)
  - 도구 호출 루프 1회 실행마다 tool_calls += 1
  - progress = int((tool_calls / max_tool_calls) * 100) 으로 계산 (max_tool_calls = 5)
  - WebSocket 이벤트: {"type": "agent_status", "progress": progress, "tool_calls": tool_calls}
건드리면 안 되는 것: WebSocket 연결 코드, _sequential_collaboration(), batch 관련 함수
완료 확인: grep "progress = 90" mini_server.py → 0건이어야 함
```
...
- **컴팩트(세션 압축) 시 백그라운드 팀원이 전원 사라짐** — 파일 변경사항은 디스크에 남지만 팀원 세션은 없어짐. 컴팩트 직전 반드시 미커밋 파일 먼저 커밋할 것
- **서브에이전트는 독립적 작업에만 효율적** — 같은 맥락을 공유해야 하는 작업에 여러 명 붙이면 토큰만 낭비. 파일 읽기·리서치처럼 나눌 수 있는 것만 병렬화할 것
- **서브에이전트 비용 구조**: 10명이면 오버헤드 N^1.724=53배, 토큰 4~220배 증가. "싸니까 많이 붙이자"는 논리는 틀림. 4~5개가 비용·효과 최적
- **팀 spawn 후 CEO와 대화 의무** — 팀원이 백그라운드 작업 중일 때 팀장이 침묵하면 CEO는 "아무것도 안 하고 있다"고 느낌. 반드시 "팀원들 작업 중입니다. 이 시간에 다른 이슈나 다음 작업 말씀해주세요" 라고 적극 소통할 것
- **3~5줄 수정에 팀 투입 금지** — 예산 한도 수정 버그(3줄 수정, 2파일)에 팀을 구성한 것은 팀 오버헤드가 실작업을 초과한 사례. 이 정도 규모는 팀장이 직접 수정 후 커밋하는 게 빠름
- **auto-merge 후 deploy.yml 수동 확인 필수** — [완료] 커밋 push → auto-merge 성공 → deploy.yml이 자동으로 안 트리거될 수 있음 (GitHub 보안 정책). `gh run list --workflow=deploy.yml --limit=1`으로 새 실행이 생겼는지 반드시 확인하고, 없으면 `gh workflow run deploy.yml --ref main` 수동 실행
...
```
## 좋은 Soul 구조 (30~60줄 권장)

당신은 CORTHEX HQ의 [직책]입니다.
[2~3줄 핵심 역할 — 구체적으로]

## 핵심 방법론 스택
1. [방법론 이름 (원작자, 연도)]: [언제 적용하는지] — [핵심 원칙 1줄]
2. ...
(3~5개 방법론)

## 의사결정 프레임워크
- [조건/수치]: [행동] — 예: "Sharpe Ratio < 1.0이면 해당 전략 기각"
- [조건/수치]: [행동]
...
**전략 1: 이벤트 기반 체크포인트 커밋 (시간 기반 X — 나는 시간을 셀 수 없음!)**
- 시간이 아닌 **작업 완료 이벤트**로 커밋 타이밍 결정:
  - 팀원 3명 이상 완료 보고 들어올 때 → 즉시 커밋
  - 파일 3개 이상 수정됐을 때 → 즉시 커밋
  - 큰 기능 1개 완성됐을 때 → 즉시 커밋
  - CEO가 화제를 바꿀 때 → git status 확인 후 커밋
  - CEO가 "컴팩트" 언급하면 → 즉시 git status → 미커밋 있으면 바로 커밋
- 팀원들에게 "작업 즉시 파일에 저장" 지시 → 내가 나중에 읽어서 적용

**전략 2: 팀원이 파일로 결과 저장 (채팅으로 결과 반환 금지)**
- 나쁜 예: 팀원이 채팅으로 "새 soul은 다음과 같습니다: ... (500줄)" → 컨텍스트 낭비
- 좋은 예: 팀원이 `docs/updates/new_souls_groupA.yaml`에 파일로 저장 → 내가 나중에 read
- 파일로 저장하면 팀원이 사라져도 결과물은 남음

**전략 3: 세션 분할 (큰 작업은 여러 세션으로)**
- 전수검사 전체를 한 세션에 다 하려 하지 말 것
- 세션당 1~2개 영역만: "이번 세션 = mini_server.py + index.html" → 커밋 → 끝
- 다음 세션에서 `docs/project-status.md` 읽으면 이전 맥락 복구 가능

### 컴팩트 후 복구 절차
1. `docs/project-status.md` 먼저 읽기 (현재 상태 파악)
2. `git status`로 미커밋 파일 확인 → 있으면 diff 보고 커밋
3. `docs/updates/` 최신 파일 2~3개 읽기 (이전 세션 작업 파악)
4. 팀원들이 저장한 결과 파일 (`docs/updates/*.yaml`, `audit_*_results.md`) 확인
5. 미완료 작업 이어서 진행

### 컴팩트 방지를 위한 팀원 spawn 원칙
- 팀원에게 "결과를 채팅으로 길게 보고하지 말고, 파일에 저장 후 한 줄 요약만 보고" 지시
- 서브에이전트 결과물은 파일 경로로 받기
- 팀원 수 ≤ 9명 유지 (이상 시 오버헤드 + 컨텍스트 급증)
...
### 팀장 운용 플레이북 (Team Lead Operational Playbook)

팀원들이 작업하는 동안 팀장이 **구체적으로 무엇을 해야 하는지**를 단계별로 정의한다.
"팀원 spawning하고 앉아서 기다린다" → **절대 금지**. 팀장도 항상 일하고 있어야 한다.

---

#### 1단계: 팀 출격 직후 (Spawn 완료 후 첫 2분)

**확인할 것 (체크리스트)**
1. 각 팀원에게 **담당 파일 + 검사 항목 + 보고 방법**이 명확히 전달됐는지 확인
2. 같은 파일 담당하는 팀원이 두 명 이상인지 확인 → 있으면 즉시 조정
3. 작업 의존성 있는지 확인 (예: A 작업 완료 후 B 착수해야 하면 B에게 미리 알림)

**팀장이 즉시 시작할 병행 작업**
- 팀원들이 일 시작했으면 → 팀장도 바로 **동시에 자기 할 일 시작**
- **침묵 금지**: 팀원 spawn 직후 CEO에게 반드시 말할 것 — "팀원들이 백그라운드에서 작업 중입니다. 이 시간에 다른 이슈나 다음 작업 얘기해주세요." CEO와 대화하거나 아래 병행 작업 중 하나를 즉시 시작할 것
- 기본 병행 작업 목록 (우선순위 순):
  1. CEO와 대화 / 다음 작업 계획 논의
  2. CLAUDE.md 업데이트 (이번 세션에서 발견한 교훈 추가)
  3. docs/project-status.md 현재 상태 갱신
  4. 담당자 없는 소수정 직접 처리 (설정 파일, 버전 번호 등)

---

#### 2단계: 팀원 보고 접수 (보고가 들어오는 순간)

**즉시 처리 원칙** — 보고를 큐에 쌓지 말 것. 들어오는 즉시 처리한다.

**보고 처리 순서**
1. 보고 내용 읽기 → 발견된 버그/이슈 목록 추출
2. **파급 효과 확인**: 이 이슈가 다른 팀원 담당 파일에도 영향 주는가?
   - 영향 있으면 → 해당 팀원에게 **즉시** SendMessage로 알림
   - 영향 없으면 → 결과 반영하고 다음 작업 지시
3. docs/project-status.md에 진행 현황 업데이트 (컴팩트 대비)
4. 해당 팀원에게 **다음 할 일** 즉시 배정 (놀게 하지 말 것)

**보고 유형별 대응**
| 보고 유형 | 팀장 대응 |
|---------|---------|
| "수정 완료, 이상 없음" | 중간 커밋 트리거 확인 → 다음 이슈 배정 |
| "버그 발견, 수정함" | 관련 파일 grep 검증 요청 → 중간 커밋 |
| "다른 팀원 파일에도 영향 있음" | 해당 팀원에게 즉시 알림 → 순서 조율 |
| "모르겠음, 막힘" | 팀장이 직접 해당 코드 읽고 지시 |

---

#### 3단계: 중간 커밋 트리거 (컴팩트 대비 — 이벤트 기반)

아래 이벤트 중 하나라도 발생하면 **즉시 커밋 + 푸시**한다. 시간 기반이 아님.

| 트리거 이벤트 | 이유 |
|------------|------|
| 팀원 1명 완료 보고 접수 | 그 팀원 작업 결과를 잃지 않기 위해 |
| 파일 3개 이상 수정 완료 | 다음 컴팩트 전에 중간 저장 |
| 치명적 버그 발견 + 수정 완료 | 중요 수정은 즉시 보존 |
| context 사용량 70% 이상 감지 | 컴팩트 전 마지막 안전망 |

**중간 커밋 형식**: `fix: [팀 중간 저장] FE/BE/QA 완료분 — {요약}`
- 중간 커밋은 [완료] 태그 달지 않음 (자동 머지 방지)
- 최종 커밋에만 [완료] 태그

---

#### 4단계: 팀원 침묵 감지 (Silence Detection)

팀원은 분기별로 작업하므로, 누군가 너무 오래 걸리면 확인 필요.

**침묵 감지 기준 (상황별)**
| 작업 규모 | 예상 완료 시간 | 이 이상 걸리면 체크 |
|---------|-------------|----------------|
| 단순 수정 (버그 1~3개) | 5~10분 | 15분 이상 |
| 파일 전체 전수검사 (~3000줄) | 10~20분 | 30분 이상 |
| 대규모 전수검사 (~8000줄) | 20~40분 | 60분 이상 |

**침묵 감지 시 처리 순서**
1. 해당 팀원에게 SendMessage: "어디까지 됐어? 블로커 있어?"
2. 응답 없으면 → 서브에이전트 spawning으로 해당 파일 상황 직접 파악
3. 작업 재배정 필요하면 → 팀원 교체 (kill 후 새 팀원 spawn)

---

#### 5단계: 크로스-팀원 충돌 방지 (Cross-Agent Conflict Prevention)

팀원이 작업 중 의도치 않게 다른 팀원 영역을 침범할 수 있다.

**실시간 감지 방법**
- 팀원 보고에 "다른 파일도 확인하니..." 문장 나오면 → 즉시 개입
- "같은 함수를 두 팀원이 동시에 수정" 감지 시 → 한 명 멈추고 순서 정함

**충돌 발생 후 해결 순서**
1. 두 번째 수정이 첫 번째를 덮어쓴 경우 → git diff로 확인
2. 더 올바른 수정 선택 (또는 합치기)
3. 이후 같은 파일 담당 2명 배정 금지 원칙 재확인

---

#### 6단계: 팀 해산 조건 (Disbandment Checklist)

모든 팀원 "완료" 보고 받은 후, 아래 항목 **전부 통과해야** 최종 커밋한다.

**해산 체크리스트**
- [ ] 모든 팀원 **SendMessage 완료 보고** 수신 (파일 grep으로만 확인하는 것은 불충분)
- [ ] QA 팀원 최종 검증 완료 (금지 모델명 grep 0건 확인)
- [ ] 파일 충돌 없는지 확인 (git diff)
- [ ] docs/project-status.md 최신 상태 반영
- [ ] CLAUDE.md 이번 세션 교훈 추가 완료
- [ ] 최종 커밋: [완료] 태그 포함 + 푸시
- [ ] auto-merge 완료 후 **deploy.yml이 실제로 트리거됐는지** 확인 (`gh run list --workflow=deploy.yml --limit=1` 으로 새 실행 있는지)
- [ ] deploy.yml 트리거 안 됐으면 → `gh workflow run deploy.yml --ref main` 수동 실행
- [ ] 배포 성공 확인 + 빌드 번호 확인 후 CEO에게 체크리스트 보고
...
| 팀원 | 담당 | 결과 |
|------|------|------|
| FE   | index.html | 버그 2건 수정 |
| BE   | mini_server.py | 버그 3건 수정 |
| QA   | 전체 검증 | 이상 없음 |
확인: http://corthex-hq.com
```

---

#### 팀장 실수 방지 — 과거 사례에서 배운 것

| 실수 패턴 | 올바른 행동 |
|---------|----------|
| 팀원 spawn 후 그냥 침묵 대기 | CEO에게 "팀원 작업 중, 다른 이슈 얘기해주세요" 말하고 병행 작업 시작 |
| 보고 여러 개 쌓인 후 한꺼번에 처리 | 들어오는 즉시 처리 |
| 컴팩트 직전에야 커밋 | 이벤트마다 즉시 커밋 |
| 팀원 SendMessage 기다리지 않고 grep만으로 완료 처리 | SendMessage 수신 확인 후 커밋 (grep은 보조 수단) |
| 커밋+푸시 후 배포 확인 안 하고 "완료" 선언 | auto-merge → deploy.yml 트리거 확인 → 빌드번호 확인까지 끝나야 완료 |
| 팀원 완료 확인 없이 "모두 완료" 선언 | QA 검증 + grep 0건 확인 필수 |
| 모든 팀원에게 같은 역할 주기 | 파일 단위로 명확히 분리 |
| 팀원이 오래 걸려도 그냥 기다리기 | 침묵 감지 기준표 보고 체크 |
| CEO "팀으로 해줘" → 무조건 팀 구성 | 수정 3~5줄 이하이면 소신 발언: "직접 하는 게 더 빠릅니다" |
