...
##### 팀 편성 기준표 (근거: Amdahl's Law + 2024 LLM 실증 연구)
...
- **팀 spawn 직후**: 같은 파일 담당 2명 없는지 확인 → CEO에게 "팀원 작업 중입니다. 다른 이슈나 다음 작업 얘기해주세요" 즉시 말할 것 (침묵 금지)
- **보고 받으면**: 즉시 처리. 다른 팀원 파일 영향 있으면 해당 팀원에게 SendMessage
- **중간 커밋 트리거**: 팀원 1명 완료 보고 / 파일 3개 이상 수정 / 치명적 버그 수정 → 즉시 커밋+푸시 (중간 커밋에 [완료] 태그 금지)
- **침묵 감지**: 단순 수정 15분, 전수검사 30분 이상이면 SendMessage로 확인
- **충돌 방지**: 팀원 보고에 "다른 파일도 확인하니..." 나오면 즉시 개입
