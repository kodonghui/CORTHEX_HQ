...
**"됐겠지" 절대 금지. 내가 먼저 시나리오로 검증한다.**

```
[단계 A] → [단계 B] → [단계 C] = 기댓값 먼저 말하고 → 실제 확인
```

1. 핵심 플로우 시나리오 3개 이상 직접 작성
2. 각 시나리오마다 기댓값 명시 후 실서버에서 확인
3. 기댓값 ≠ 실제 → 즉시 self-doctor (investigator → 수정 → 재검증)
4. 대표님 보고 전 완료. 보고는 선택.

> 상세: `docs/claude-rules/코딩_개발.md` → 소크라테스 QA 섹션
