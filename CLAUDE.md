...
#### 2단계: 비용 vs 효과 최적화 — 수학적 근거

**팀 편성 전 반드시 이 계산을 먼저 한다. 팀 뽑고 나서 계산하면 이미 늦음.**

##### 법칙 1: Amdahl's Law (암달의 법칙) — 병렬화 한계
...
전수검사 기준:
- 파일 읽기(병렬 가능): 약 70%  → P = 0.7
- 수정/판단(병렬 불가): 약 30%  → 1-P = 0.3
```

| 서브에이전트 수(N) | 속도 향상 S(N) | 추가 투입 대비 추가 향상 | ROI |
|-----------------|-------------|---------------------|-----|
| 1 (기준) | 1.00배 | — | 기준 |
| 2 | 1.43배 | **+43%** | 0.71 |
| 3 | 1.79배 | +25% | 0.60 |
| **4** | **2.06배** | **+15%** | **0.51 ← 여기가 변곡점** |
| 5 | 2.27배 | +10% | 0.45 |
| 10 | 2.70배 | +19% (5→10 합산) | 0.27 |
| 20 | 2.99배 | +11% (10→20 합산) | 0.15 |

**결론**: N=4~5가 최적. N=5 이후부터 돈은 2배 쓰는데 속도는 10% 밖에 안 올라감.

##### 법칙 2: Brooks' Law (브룩스의 법칙) — 팀원 늘릴수록 오버헤드 증가

```
커뮤니케이션 경로 수 = N(N-1)/2

N=5:  10개 경로
N=10: 45개 경로  ← 5배 늘렸는데 경로는 4.5배 증가
N=15: 105개 경로
N=20: 190개 경로 ← 실제 처리량보다 조율 비용이 더 커짐
```

→ 팀원은 적게, 서브에이전트는 많게. 팀원 10명 + 서브 3명이 팀원 30명보다 효율적.

##### 법칙 3: 수확 체감의 법칙 (Diminishing Returns)

파일 읽기는 P=0.7이라 이론적 최대 속도 향상 = 1/(1-0.7) = **3.33배**가 천장.
서브에이전트를 무한히 늘려도 3.33배 이상은 절대 안 됨. N=5면 이미 68% 달성.

##### 최종 공식 — 팀 편성 전 이걸 계산

```
서브에이전트 최적 수:
- 파일 500줄 이하   → 서브에이전트 2명 (N=2, ROI 0.71로 가성비 최고)
- 파일 500~3000줄  → 서브에이전트 3명 (N=3, 속도 1.79배)
- 파일 3000줄 이상 → 서브에이전트 4명 (N=4, 변곡점. 이 이상은 수확체감 급격)
- 파일 5000줄 이상 → 서브에이전트 5명 (N=5, 속도 2.27배. 여기서 멈출 것)

팀원 최적 수:
- 브룩스 법칙상 팀원 9명 초과 시 오버헤드 급증
- 파일 수 = 팀원 수 (1파일 1담당) + QA 1명이 기본
- 전체 전수검사도 팀원 10~12명이 실용적 상한선
```

| 작업 규모 | 팀원 수 | 서브에이전트 | 총 에이전트 | 이유 |
|----------|--------|------------|-----------|------|
| 단일 버그 수정 | 1~2명 + QA | 2~3명 | 6~9개 | 팀 오버헤드가 작업보다 큼 |
| 기능 영역 점검 | 3~5명 + QA | 3명씩 | 12~18개 | Amdahl N=3 최적 |
| 전체 전수검사 | 파일 수 + QA (보통 10~12명) | 파일 크기별 3~4명 | 40~50개 | 브룩스 법칙 상한 고려 |
| 도구 대규모 검사 | TOOL 1명 + 서브 5명 | 5명 (도구 그룹별) | 6개 | 팀원 늘리면 브룩스 함정 |
