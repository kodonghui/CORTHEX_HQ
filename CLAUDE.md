...
### 전수검사 표준 팀 구성 (팀원 수는 작업 규모에 따라 자유롭게)

**언제 쓰는가**: CEO가 "전수검사 해줘", "처음부터 끝까지 다 읽어서 고쳐줘", "전체 점검"이라고 하면 반드시 팀 구성으로 진행할 것. 혼자 하면 반드시 누락 발생.

**팀 규모 결정 방법 — Claude가 판단해서 결정 (CEO가 지정하면 그게 우선)**

CEO가 인원수를 지정하지 않으면, 나(Claude)가 아래 공식으로 최적 규모를 판단:

```
공식:
(수정 대상 파일 수) + (주요 도메인 수) + QA 1명 = 기본 팀원 수

예시:
- 전체 코드베이스 전수검사 → 파일 9개 + 도메인(배치/SNS/TG) → 12~15명
- 배치 시스템만 → 관련 파일 3개 → 4명 (AI + BE + FE + QA)
- 단일 버그 수정 → 1~3명
- 도구 154개 개별 검사 → 도구 수에 맞게 20~30명도 가능
```

**서브에이전트 수도 파일 크기에 따라 판단:**
- 500줄 이하 파일 → 서브에이전트 2명
- 500~3000줄 파일 → 서브에이전트 3명
- 3000줄 이상 파일 (index.html 8000줄, mini_server.py 7000줄 등) → 서브에이전트 4~5명

**팀 구성 표 (파일별 담당 1명 — 충돌 방지)**

| 코드명 | 담당 파일 (이 파일만 수정) | 검사 핵심 |
|--------|--------------------------|-----------|
| **FE** | `web/templates/index.html` | SNS 연결 상태, 에이전트 카드, 배치 탭, 작전현황, 사령관실, 하드코딩 배열, 다크모드 |
| **BE** | `web/mini_server.py` | API 중복 정의, 에이전트 목록 동기화, 배치 엔드포인트, async/await |
| **AI** | `web/ai_handler.py` + `src/llm/` | 배치 SDK 형식, 모델명, _PRICING 가격표, max_tokens |
| **TG** | `src/telegram/` | 텔레그램 봇 명령어, 배치 연동, 새 기능 (/result, /cancel, /models 등) |
| **AGENT** | `src/core/` | 에이전트 soul, quality_gate, 도구 루프 최대 5회, 모델명 |
| **TOOL** | `src/tools/` | 도구 스키마 일치, SNS 도구, Instagram 오류 원인, Selenium 설정 |
| **SNS** | `src/integrations/` | SNS 발행 로직, Instagram API, 환경변수 체크, Tistory 처리 |
| **CONFIG** | `config/` | agents.yaml 모델명, models.yaml, yaml2json.py 변환 목록 완전성 |
| **DEVOPS** | `.github/workflows/` | deploy.yml (git pull 금지 → fetch+reset), 환경변수 전달, Selenium 설치 |
| **QA** | `CLAUDE.md`, `docs/` | 금지 모델명 0건 확인, 하드코딩 잔재, CLAUDE.md 업데이트, 작업기록 작성 |

**서브에이전트 활용법 (팀원마다 3명)**
- 팀원은 먼저 Task 도구로 Explore 타입 서브에이전트 3명을 동시에 spawn
- 서브에이전트들은 담당 파일을 **병렬로 읽기** (예: FE는 index.html을 3개 구간으로 나눠서 동시 읽기)
- 서브에이전트가 읽은 결과를 받아서, 팀원은 **수정에 집중**
- 이렇게 하면 6000줄 파일도 서브에이전트 3명이 2000줄씩 동시에 읽어서 1/3 시간으로 끝남

**팀 작업 절차**
1. TeamCreate로 팀 생성
2. Task 도구로 팀원 전원을 **한 번에 동시 spawn** (run_in_background=true)
3. 각 팀원은 서브에이전트 3명 spawn → 병렬 조사 → 결과 받으면 수정 실행
4. 팀원은 작업 완료 후 SendMessage로 팀장에게 보고
5. 팀장(Claude 본인)은 모든 팀원 결과 취합 → 커밋/푸시/배포

**팀원 spawn 시 반드시 포함할 정보**
- 담당 파일 (이 파일만 수정할 것)
- 검사 항목 상세 목록
- 허용/금지 모델명 목록
- 작업 경로 (절대경로)
- "작업 완료 후 팀장에게 SendMessage로 보고" 지시

### 이번 전수검사(2026-02-18)에서 발견한 패턴들

**패턴 1: 의존성(패키지)은 반드시 먼저 확인하고 설치 지시**
- selenium이 requirements.txt에 이미 있었는데 CEO가 "설치해"라고 해서 확인해보니 이미 존재
- **교훈**: 설치/추가 작업 전 반드시 기존 파일 확인. "이미 있는지" 먼저 확인하고 보고할 것
- 확인 명령: `grep -i selenium requirements.txt`

**패턴 2: 서버 API 응답 구조 ↔ 프론트 접근 코드 불일치**
- 서버는 `{"instagram": {"connected": true}}` 형태로 반환
- 프론트는 `sns.oauthStatus.platforms[platform].connected` 형태로 접근 (다른 경로!)
- **교훈**: API 응답 구조 수정 시 → 프론트에서 해당 데이터를 어떻게 접근하는지 반드시 추적
- **전수검사 시**: 서버 API 코드와 프론트 코드를 **동시에** 열어서 데이터 흐름 대조

**패턴 3: 하드코딩 배열이 여러 파일에 흩어짐**
- 에이전트 ID 배열이 index.html 14곳에 하드코딩 → agents.yaml 변경 시 14곳 모두 변경 필요
- 플랫폼 목록(`["instagram", "youtube", "threads", "linkedin"]`)도 여러 파일에 하드코딩
- threads, linkedin 제거했는데 index.html 배열 한 곳에 남아있었음
- **교훈**: 배열/목록은 단일 소스에서 동적으로 읽어야 함. 전수검사 시 배열 리터럴(`[`, `]`) 있는 곳 전부 추적

**패턴 4: AI 에이전트의 "있을 것 같은" 모델명 지어내기**
- QA 에이전트가 `claude-haiku-4-5-20251001`을 보고 "haiku 4.6이 더 최신이겠지"라고 판단
- 실제로 존재하지 않는 `claude-haiku-4-6`을 10개 파일에 심음 → API 오류 발생
- **교훈**: 모델명은 절대 추측하지 말 것. CLAUDE.md의 "실제 존재하는 모델명" 표가 정답

**패턴 5: 배치 SDK 호출 형식 오류는 조용히 실패**
- OpenAI file 업로드: `file=bytes_data` → TypeError (올바른 형식: `file=("batch.jsonl", bytes_data)`)
- Anthropic batch results: `await await client.messages.batches.results(...)` → TypeError
- 이런 오류는 로그에만 남고 사용자에게는 "배치 실패"로만 표시됨 → 원인 파악 어려움
- **교훈**: SDK 함수 호출 시 공식 문서 형식 반드시 확인. 배치 시스템은 실제로 실행해보기 전까지 오류 발견 어려움

**패턴 6: "배포 성공"인데 코드가 안 바뀌는 현상**
- `git pull`이 충돌 에러를 내도 스크립트는 계속 실행됨 → 옛날 코드로 새 빌드번호만 붙음
- **교훈**: deploy.yml에서 `git pull` 절대 쓰지 말 것. `git fetch + git reset --hard`만 사용

**패턴 7: 새 설정 파일 추가 시 yaml2json.py도 함께 수정**
- `config/quality_rules.yaml` 추가했는데 yaml2json.py 변환 목록에 안 넣음
- 서버에 PyYAML 없어서 JSON 없이는 데이터 로드 불가 → 빈 화면
- **교훈**: 새 .yaml 파일 만들면 반드시 yaml2json.py 변환 목록에 추가. 세트로 움직임

**패턴 8: FastAPI는 같은 경로 엔드포인트 중복 정의 시 두 번째를 무시**
- `/api/tasks/bulk`가 두 번 정의됐는데 첫 번째(구버전)가 실행됨
- 새 기능(bookmark/tag)이 두 번째 정의에 있었는데 절대 실행이 안 됨
- **교훈**: mini_server.py에서 새 API 추가 시 같은 경로가 이미 있는지 grep으로 먼저 확인

**패턴 9: CSS opacity 애니메이션은 ::before 가상 요소로 분리**
- `.bg-grid`에 `opacity: 0.03` 애니메이션 직접 걸면 글자/카드 전부 투명해짐
- 배경 무늬만 깜빡이게 하려면 `.bg-grid::before`에 걸어야 함
- **교훈**: 콘텐츠가 있는 요소에 opacity 애니메이션 직접 걸면 안 됨

**패턴 10: 팀 작업 시 같은 파일 두 명 배정 금지**
- 팀 작업 시 같은 파일을 두 명이 수정하면 충돌 — 나중에 수정한 쪽이 먼저 수정한 내용을 덮어씀
- **교훈**: 팀 구성 시 반드시 파일별 담당자 1명만 지정. 다른 팀원 파일을 건드려야 하면 팀장에게 먼저 보고
